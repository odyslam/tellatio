...
function TelegramAuthDialog({ recordId, object }: any) {
  const [step, setStep] = useState<'info' | 'phone' | 'code' | 'config' | 'complete'>('info');
      ...
      {step === 'info' && (
        <div className="space-y-4">
          <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <h3 className="font-semibold text-yellow-800 mb-2">Backend Setup Required</h3>
            <p className="text-sm text-yellow-700 mb-3">
              The Telegram integration requires a Node.js backend server to handle MTProto authentication.
            </p>
            <p className="text-sm text-yellow-700 mb-3">
              To use this feature:
            </p>
            <ol className="text-sm text-yellow-700 space-y-1 ml-4">
              <li>1. Deploy the companion backend service</li>
              <li>2. Get Telegram API credentials from my.telegram.org</li>
          ...
          <div className="text-sm text-gray-600">
            This is a preview of the authentication flow. Click continue to see the UI.
          </div>
      ...
      {step === 'phone' && (
        <div className="space-y-4">
          ...
          <div className="text-xs text-gray-500 text-center">
            Your phone number will be used only for authentication
          </div>
        </div>
      )}
      ...
      {step === 'complete' && (
        <div className="space-y-4 text-center py-8">
          ...
          <p className="text-xs text-gray-500">
            (This is a preview - backend required for actual functionality)
          </p>
        </div>
      )}
    </div>
